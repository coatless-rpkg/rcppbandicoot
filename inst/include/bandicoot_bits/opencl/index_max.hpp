...
index_max(dev_mem_t<uword> dest,
  ...
  if (dim == 0)
    {
    kernel = get_rt().cl_rt.get_kernel<eT1>(oneway_kernel_id::index_max_colwise);
    }
  else
    {
    kernel = get_rt().cl_rt.get_kernel<eT1>(oneway_kernel_id::index_max_rowwise);
    }
  ...
  coot_check_cl_error(status, "coot::opencl::index_max(): failed to run kernel");
  }
...
index_max_cube_col(dev_mem_t<uword> dest,
  ...
  coot_check_cl_error(status, "coot::opencl::index_max_cube_col(): could not set arguments for kernel");
  ...
  coot_check_cl_error(status, "coot::opencl::index_max_cube_col(): failed to run kernel");
  }
...
index_max_vec(dev_mem_t<eT> mem, const uword n_elem, eT* max_val)
  ...
  return generic_reduce_uword_aux<eT, eT>(mem, n_elem, "index_max_vec", max_val, k, k_small, std::make_tuple(/* no extra args */));
  }
