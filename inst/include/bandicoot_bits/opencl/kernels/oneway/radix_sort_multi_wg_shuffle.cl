...
__kernel
void
COOT_FN(PREFIX,radix_sort_multi_wg_shuffle)(__global eT1* A,
  ...
  if (sort_type == 0)
    ...
    local_offsets[0] = counts[tid                  ]; // first place we should put a 00 point
    local_offsets[1] = counts[tid +     num_threads]; // first place we should put a 01 point
    ...
    local_offsets[0] = counts[tid + 3 * num_threads]; // first place we should put a 00 point
    local_offsets[1] = counts[tid + 2 * num_threads]; // first place we should put a 01 point
    ...
    local_offsets[2] = counts[tid                  ]; // first place we should put a 10 point
    local_offsets[3] = counts[tid     + num_threads]; // first place we should put a 11 point
    }
  else if (sort_type == 3)
    {
    ...
    local_offsets[2] = counts[tid + 3 * num_threads]; // first place we should put a 10 point
    local_offsets[3] = counts[tid + 2 * num_threads]; // first place we should put a 11 point
    }
  else if (sort_type == 4)
    {
    ...
    local_offsets[0] = counts[tid + 2 * num_threads]; // first place we should put a 00 point
    local_offsets[1] = counts[tid + 3 * num_threads]; // first place we should put a 01 point
    local_offsets[2] = counts[2 * num_threads] - counts[tid     + num_threads]; // first place we should put a 10 point
    local_offsets[3] = counts[    num_threads] - counts[tid                  ]; // first place we should put a 11 point
    ...
    local_offsets[0] = counts[tid +     num_threads]; // first place we should put a 00 point
    local_offsets[1] = counts[tid                  ]; // first place we should put a 01 point
    local_offsets[2] = counts[3 * num_threads] - (counts[tid + 2 * num_threads] - counts[2 * num_threads]); // first place we should put a 10 point
    local_offsets[3] = n_elem                  - (counts[tid + 3 * num_threads] - counts[3 * num_threads]); // first place we should put a 11 point
  ...
  if (i < end_elem)
    ...
    const UWORD out_index = local_offsets[loc];
    local_offsets[loc] += ((loc >= 2) ? upper_bit_shift : 1);
    out[out_offset + out_index] = A[A_offset + i];
    }
  }
