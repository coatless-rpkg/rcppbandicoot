...
logspace(dev_mem_t<eT> mem, const uword mem_incr, const eT start, const eT end, const uword num)
  ...
  coot_check_runtime_error( (get_rt().cl_rt.is_valid() == false), "opencl::logspace(): OpenCL runtime not valid");

  cl_kernel kernel = get_rt().cl_rt.get_kernel<eT>(oneway_kernel_id::logspace);
  ...
  const ceT step = to_cl_type(eT((double(end) - double(start)) / (double(num) - 1)));
  ...
  status |= coot_wrapper(clSetKernelArg)(kernel, 5, sizeof(ceT),      &step);
  ...
  coot_check_runtime_error( (status != CL_SUCCESS), "coot::opencl::logspace(): couldn't execute kernel");
  }
