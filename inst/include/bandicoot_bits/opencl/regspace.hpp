...
regspace(dev_mem_t<eT> mem, const uword mem_incr, const eT start, const eT delta, const eT end, const uword num, const bool desc)
  ...
  coot_check_runtime_error( (get_rt().cl_rt.is_valid() == false), "opencl::regspace(): OpenCL runtime not valid");

  cl_kernel kernel = desc ? get_rt().cl_rt.get_kernel<eT>(oneway_kernel_id::regspace_desc) 
                          : get_rt().cl_rt.get_kernel<eT>(oneway_kernel_id::linspace);
  ...
  const ceT cl_start = to_cl_type(start);
  const ceT cl_delta = to_cl_type(delta);
  const ceT cl_end = to_cl_type(end);
  ...
  status |= coot_wrapper(clSetKernelArg)(kernel, 3, sizeof(ceT),      &cl_start);
  status |= coot_wrapper(clSetKernelArg)(kernel, 4, sizeof(ceT),      &cl_end);
  status |= coot_wrapper(clSetKernelArg)(kernel, 5, sizeof(ceT),      &cl_delta);
  ...
  coot_check_runtime_error( (status != CL_SUCCESS), "coot::opencl::regspace(): couldn't execute kernel");
  }
