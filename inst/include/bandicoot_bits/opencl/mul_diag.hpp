...
mul_diag
  ...
  if (A_is_diag && !B_is_diag && !B_trans)
    ...
    diag_arg_offset = A_mem.cl_mem_ptr.offset;
    mat_arg_offset  = B_mem.cl_mem_ptr.offset;
    ...
    diag_arg_offset = B_mem.cl_mem_ptr.offset;
    mat_arg_offset  = A_mem.cl_mem_ptr.offset;
  ...
  status |= coot_wrapper(clSetKernelArg)(kernel, 4, sizeof(cl_mem),          &(mat_arg_ptr)         );
  status |= coot_wrapper(clSetKernelArg)(kernel, 5, cl_mat_arg_offset.size,  cl_mat_arg_offset.addr );
  status |= coot_wrapper(clSetKernelArg)(kernel, 6, sizeof(eT),              &alpha                 );
  status |= coot_wrapper(clSetKernelArg)(kernel, 7, cl_n_rows.size,          cl_n_rows.addr         );
  status |= coot_wrapper(clSetKernelArg)(kernel, 8, cl_n_cols.size,          cl_n_cols.addr         );
