...
// division of BaseCube objects with same element type
...
// division of Base objects with different element types
template<typename T1, typename T2>
inline
typename
enable_if2
  ...
  const mtGlueCube<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, mtglue_mixed_core<mtglue_mixed_div> >
  >::result
operator/
  (
  const T1& X,
  ...
  return mtGlueCube<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, mtglue_mixed_core<mtglue_mixed_div> >( X, Y );
  }
