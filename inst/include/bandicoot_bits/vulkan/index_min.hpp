...
index_min(dev_mem_t<uword> dest,
  ...
  if (!(is_float<eT>::value || is_double<eT>::value))
    {
    coot_stop_runtime_error("coot::vulkan::index_min(): unsupported type");
    }
  ...
  pipeline_t& pipe = (dim == 0)
      ? rt.get_gen_index_min_colwise_pipeline<eT>()
      : rt.get_gen_index_min_rowwise_pipeline<eT>();
...
inline
uword
index_min_vec(const dev_mem_t<eT> mem, const uword n_elem, eT* min_val)
  {
  coot_debug_sigprint();

  if (n_elem == 0) { if (min_val) { *min_val = eT(0); } return 0; }

  if (!(is_float<eT>::value || is_double<eT>::value))
    {
    coot_stop_runtime_error("coot::vulkan::index_min_vec(): unsupported type");
    }
  ...
  pipeline_t& pipe = rt.get_gen_index_min_vec_pipeline<eT>();
