...
op_min::apply_noalias(Cube<out_eT>& out, const Cube<in_eT>& A, const uword dim, const bool post_conv_apply)
  ...
  if (dim == 0)
    ...
    coot_rt_t::min_cube_col(out.get_dev_mem(false), A.get_dev_mem(false),
                            A.n_rows, A.n_cols, A.n_slices,
                            post_conv_apply);
    }
  }
...
inline
uword
op_min::compute_n_rows(const OpCube<T1, op_min>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
  {
  coot_ignore(in_n_cols);
...
inline
uword
op_min::compute_n_cols(const OpCube<T1, op_min>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
  {
  coot_ignore(in_n_rows);
  coot_ignore(in_n_slices);
...
inline
uword
op_min::compute_n_slices(const OpCube<T1, op_min>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
  {
  coot_ignore(in_n_rows);
  coot_ignore(in_n_cols);
