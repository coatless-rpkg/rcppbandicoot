...
__kernel
void
COOT_FN(PREFIX,equ_array_div_scalar_pre_sve2)(__global eT2* dest,
  ...
  if (row < n_row_elems && col < n_col_elems)
    ...
    if (val_post == (eT2) (0))
      ...
      dest[dest_loc] = (eT2) (val_pre / src[src_loc]);
      }
    else if (val_pre == (eT1) (0) && val_post != (eT2) (0))
      {
      dest[dest_loc] = val_post / ((eT2) src[src_loc]);
      }
    else
      {
      // If both are nonzero, we apply sequentially---be careful!
      dest[dest_loc] = val_post / ((eT2) (val_pre / src[src_loc]));
      }
    }
  }
