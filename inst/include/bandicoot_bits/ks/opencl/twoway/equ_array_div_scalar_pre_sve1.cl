...
__kernel
void
COOT_FN(PREFIX,equ_array_div_scalar_pre_sve1)(__global eT2* dest,
  ...
  if (i < n_elem)
    ...
    if (val_post == (eT2) (0))
      ...
      {
      dest[dest_locs[i + dest_locs_offset] + dest_offset] = val_post / ((eT2) src[src_locs[i + src_locs_offset] + src_offset]);
      }
    else
      {
      // if both are nonzero, we apply sequentially---be careful!
      dest[dest_locs[i + dest_locs_offset] + dest_offset] = val_post / ((eT2) (val_pre / src[src_locs[i + src_locs_offset] + src_offset]));
      }
    }
  }
