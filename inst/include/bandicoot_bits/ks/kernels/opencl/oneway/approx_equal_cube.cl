...
__kernel
void
COOT_FN(PREFIX,approx_equal_cube)(__global uint* out_mem,
  ...
  while (i + get_local_size(0) < n_elem)
    ...
    const UWORD elem2 = (i + get_local_size(0)) % n_elem_slice;
    const UWORD slice2 = (i + get_local_size(0)) / n_elem_slice;
    const UWORD row2 = elem2 % n_rows;
    const UWORD col2 = elem2 / n_rows;

    const UWORD A_loc1 = A_offset + row1 + col1 * A_M_n_rows + slice1 * A_M_n_rows * A_M_n_cols;
    const UWORD A_loc2 = A_offset + row2 + col2 * A_M_n_rows + slice2 * A_M_n_rows * A_M_n_cols;
    const UWORD B_loc1 = B_offset + row1 + col1 * B_M_n_rows + slice1 * B_M_n_rows * B_M_n_cols;
    const UWORD B_loc2 = B_offset + row2 + col2 * B_M_n_rows + slice2 * B_M_n_rows * B_M_n_cols;
  ...
  if (i < n_elem)
    ...
    const UWORD A_loc = A_offset + row + col * A_M_n_rows + slice * A_M_n_rows * A_M_n_cols;
    const UWORD B_loc = B_offset + row + col * B_M_n_rows + slice * B_M_n_rows * B_M_n_cols;
