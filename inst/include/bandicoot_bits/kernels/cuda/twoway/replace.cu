...
__global__
void
COOT_FN(PREFIX,replace)(eT2* dest,
                        eT1* src,
                        const eT1 val_find,
  ...
  if (row < n_rows && col < n_cols && slice < n_slices)
    ...
    if (coot_isnan(val_find))
      {
      // We are searching for a NaN so the check is a little different.
      dest[dest_index] = TO_ET2((coot_isnan(val)) ? val_replace : val);
      }
    else
      {
      // No special handling needed.
      dest[dest_index] = TO_ET2((val == val_find) ? val_replace : val);
      }
    }
  }
