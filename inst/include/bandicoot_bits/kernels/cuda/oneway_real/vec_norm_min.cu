...
__device__ void COOT_FN(PREFIX,min_subgroup_reduce)(volatile eT1* data, int tid);
...
COOT_FN(PREFIX,vec_norm_min)(const eT1* in_mem,
  ...
  if (tid < 32) // unroll last warp's worth of work
    {
    // Since we are just accumulating, we can use the min_subgroup_reduce utility function.
    COOT_FN(PREFIX,min_subgroup_reduce)(aux_mem, tid);
    }
