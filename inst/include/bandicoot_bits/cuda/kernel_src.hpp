...
struct kernel_src
  ...
  static inline std::vector<std::string> get_defs(const bool has_fp16);
...
kernel_src::get_defs(const bool has_fp16)
  {
  ...
  opts.push_back("-DCOOT_PI=" STR2(M_PI));

  opts.push_back("-DCOOT_S8_MIN="  + std::string(s8_min));
  opts.push_back("-DCOOT_S16_MIN=" + std::string(s16_min));
  opts.push_back("-DCOOT_S32_MIN=" + std::string(s32_min));
  opts.push_back("-DCOOT_S64_MIN=" + std::string(s64_min));

  opts.push_back("-DCOOT_U8_MAX="  + std::string(u8_max));
  opts.push_back("-DCOOT_U16_MAX=" + std::string(u16_max));
  opts.push_back("-DCOOT_U32_MAX=" + std::string(u32_max));
  opts.push_back("-DCOOT_U64_MAX=" + std::string(u64_max));
  opts.push_back("-DCOOT_S8_MAX="  + std::string(s8_max));
  opts.push_back("-DCOOT_S16_MAX=" + std::string(s16_max));
  opts.push_back("-DCOOT_S32_MAX=" + std::string(s32_max));
  opts.push_back("-DCOOT_S64_MAX=" + std::string(s64_max));

  if (has_fp16)
    {
    opts.push_back("-DCOOT_HAVE_FP16");
    opts.push_back("-DHALF_MIN=" STR2(CUDART_MIN_DENORM_FP16));
    opts.push_back("-DHALF_MAX=" STR2(CUDART_MAX_NORMAL_FP16));
    }

  opts.push_back("-DFLT_MIN=" STR2(FLT_MIN));
  opts.push_back("-DFLT_MAX=" STR2(FLT_MAX));
  opts.push_back("-DDBL_MIN=" STR2(DBL_MIN));
  opts.push_back("-DDBL_MAX=" STR2(DBL_MAX));
  opts.push_back("-DSIZE_MAX=" STR2(SIZE_MAX));
...
kernel_src::get_oneway_defines()
  ...
  source += read_file("defs/" + rt_type_prefix<eT>() + "_defs.cu");
  if (is_same_type<eT, fp_eT>::no)
    source += read_file("defs/" + rt_type_prefix<fp_eT>() + "_defs.cu");
  if (is_same_type<eT, uint_eT>::no)
    source += read_file("defs/" + rt_type_prefix<uint_eT>() + "_defs.cu");
...
kernel_src::get_twoway_defines()
  ...
  if (is_same_type<eT1, twoway_promoted_eT>::no &&
    ...
    source += read_file("defs/" + rt_type_prefix<twoway_promoted_eT>() + "_defs.cu");
