...
max(dev_mem_t<eT2> dest,
  ...
  coot_check_runtime_error( (get_rt().cuda_rt.is_valid() == false), "coot::cuda::max(): cuda runtime not valid" );
...
max_vec(dev_mem_t<eT> mem, const uword n_elem)
  ...
  coot_check_runtime_error( (get_rt().cuda_rt.is_valid() == false), "coot::cuda::max_vec(): cuda runtime not valid" );
...
max_cube_col(dev_mem_t<eT2> dest,
  ...
  coot_check_runtime_error( (get_rt().cuda_rt.is_valid() == false), "coot::cuda::max_cube_col(): CUDA runtime not valid" );
