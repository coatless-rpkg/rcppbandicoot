...
Col<eT>::operator=(const char* text)
  ...
  coot_conform_check( ((tmp.n_elem > 0) && (tmp.is_vec() == false)), "Mat::init(): requested size is not compatible with column vector layout" );
...
Col<eT>::rows(const uword in_row1, const uword in_row2)
  ...
  coot_conform_check_bounds( ((in_row1 > in_row2) || (in_row2 >= Mat<eT>::n_rows) ), "Col::rows(): indices out of bounds or incorrectly used");
...
Col<eT>::subvec(const uword start_row, const SizeMat& s)
  ...
  coot_conform_check( (s.n_cols != 1), "Col::subvec(): given size does not specify a column vector" );

  coot_conform_check_bounds( ( (start_row >= Mat<eT>::n_rows) || ((start_row + s.n_rows) > Mat<eT>::n_rows) ), "Col::subvec(): size out of bounds" );
