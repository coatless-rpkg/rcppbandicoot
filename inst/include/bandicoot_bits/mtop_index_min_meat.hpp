...
mtop_index_min::apply(Cube<uword>& out, const mtOpCube<uword, T1, mtop_index_min>& in)
  ...
  alias_wrapper<Cube<uword>, Cube<typename plain_unwrap_cube<T1>::stored_type::elem_type>> W(out, E.M);
  mtop_index_min::apply_noalias(W.use, E.M, dim);
  }
...
mtop_index_min::apply_noalias(Cube<uword>& out, const Cube<eT>& A, const uword dim)
  ...
  if (dim == 0)
    ...
    coot_rt_t::index_min_cube_col(out.get_dev_mem(false), A.get_dev_mem(false),
...
inline
uword
mtop_index_min::compute_n_rows(const mtOpCube<uword, T1, mtop_index_min>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
  {
  coot_ignore(in_n_cols);
...
inline
uword
mtop_index_min::compute_n_cols(const mtOpCube<uword, T1, mtop_index_min>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
  {
  coot_ignore(in_n_rows);
  coot_ignore(in_n_slices);
...
inline
uword
mtop_index_min::compute_n_slices(const mtOpCube<uword, T1, mtop_index_min>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
  {
  coot_ignore(in_n_rows);
  coot_ignore(in_n_cols);
