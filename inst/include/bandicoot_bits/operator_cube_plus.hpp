...
enable_if2
  ...
  const mtGlueCube<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, mtglue_mixed_plus>
  >::result
operator+
  (
  const T1& X,
  ...
  return mtGlueCube<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, mtglue_mixed_plus>( X, Y );
  }
